...
function TextInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className="w-full rounded-[16px] border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-[#73A5CA] focus:ring-2 focus:ring-[#73A5CA]/20" />
}

function TextArea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea {...props} rows={4} className="w-full resize-y rounded-[16px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm leading-6 text-slate-900 outline-none transition focus:border-[#73A5CA] focus:ring-2 focus:ring-[#73A5CA]/20" />
}
...
function PreviewImage({ src, alt }: { src: string; alt: string }) {
  if (!src?.trim()) return <div className="flex h-full min-h-[220px] items-center justify-center text-sm text-slate-500">Image preview</div>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
      <div className="rounded-[30px] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="text-[11px] uppercase tracking-[0.26em] text-slate-400">Services editor</div>
            <h2 className="mt-2 text-2xl font-black tracking-[-0.04em] text-slate-900">{draft.id ? 'Edit service' : 'Add a new service'}</h2>
            <p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">Add, edit, delete, and preview website service cards plus their detail pages.</p>
          </div>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-[22px] border border-slate-200 bg-slate-50 p-3">
          <div className="grid w-full gap-3 sm:grid-cols-[minmax(220px,1fr)_auto] sm:items-center">
            <label className="block">
              <span className="text-[10px] uppercase tracking-[0.24em] text-slate-400">Select service</span>
              <select
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          ...
          <Field label="Icon name">
            <select value={draft.icon_name} onChange={(event) => setDraft((prev) => ({ ...prev, icon_name: event.target.value as ServiceDraft['icon_name'] }))} className="w-full rounded-[16px] border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-[#73A5CA] focus:ring-2 focus:ring-[#73A5CA]/20">
      ...
      <div className="space-y-4">
        <div className="rounded-[30px] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
          ...
          <h3 className="mt-2 text-xl font-black text-slate-900">{draft.title || 'Service title'}</h3>
          <div className="relative mt-5 h-[360px] overflow-hidden rounded-[28px] border border-slate-200 bg-slate-50"><PreviewImage src={draft.image} alt={draft.title || 'Service preview'} /></div>
          <p className="mt-4 text-sm leading-6 text-slate-500">{draft.description || 'Service description will appear here.'}</p>
        </div>

        <div className="rounded-[30px] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-[11px] uppercase tracking-[0.26em] text-slate-400">Service library</div>
              <h3 className="mt-2 text-xl font-black tracking-[-0.04em] text-slate-900">Select a service</h3>
            </div>
            <Layers3 size={18} className="text-[#E87F24]" />
          </div>
          <div className="mt-5 space-y-3">
            {services.length ? services.map((service) => (
              <button key={service.id} type="button" onClick={() => setDraft(toDraft(service))} className="group flex w-full flex-col gap-4 rounded-[24px] border border-slate-200 bg-slate-50 p-3 text-left transition hover:-translate-y-0.5 hover:border-emerald-200 hover:bg-white/7 sm:flex-row sm:items-start">
                <div className="relative aspect-[16/10] w-full overflow-hidden rounded-[18px] border border-slate-200 sm:h-20 sm:w-28 sm:shrink-0 sm:aspect-auto"><PreviewImage src={service.image} alt={service.title} /></div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-3">
                    <h4 className="truncate text-sm font-semibold text-slate-900">{service.title}</h4>
                    <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1 text-[10px] uppercase tracking-[0.22em] text-slate-500">{service.kicker}</span>
                  </div>
                  <p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-500">{service.description}</p>
                  <div className="mt-2 text-[10px] uppercase tracking-[0.24em] text-slate-400">{service.slug}</div>
                </div>
              </button>
            )) : (
              <div className="rounded-[24px] border border-dashed border-slate-200 bg-slate-50 p-5 text-sm leading-6 text-slate-500">
                <div className="font-semibold text-slate-900">No services in Supabase yet.</div>
