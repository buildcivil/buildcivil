...
function SectionHeader({ title, copy }: { title: string; copy: string }) {
  return (
    <div className="rounded-[30px] border border-white/8 bg-[#171719] p-6 shadow-[0_18px_60px_rgba(0,0,0,0.28)]">
      <div className="flex items-center gap-3 text-[11px] uppercase tracking-[0.26em] text-white/40">
        <Sparkles size={12} className="text-[#D8FF6A]" />
        Pages CMS
      </div>
      <h2 className="mt-3 text-3xl font-black tracking-[-0.05em] text-white">{title}</h2>
      <p className="mt-3 max-w-3xl text-sm leading-7 text-white/60">{copy}</p>
    </div>
  )
}
...
function TextInput(props: InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`w-full rounded-[16px] border border-white/10 bg-[#0f0f0f] px-4 py-3 text-sm text-[#F5F3EB] outline-none transition placeholder:text-white/28 focus:border-[#D8FF6A]/70 focus:ring-2 focus:ring-[#D8FF6A]/10 ${props.className ?? ''}`}
    />
  )
}
...
function TextArea(props: TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      {...props}
      className={`min-h-[120px] w-full rounded-[16px] border border-white/10 bg-[#0f0f0f] px-4 py-3 text-sm text-[#F5F3EB] outline-none transition placeholder:text-white/28 focus:border-[#D8FF6A]/70 focus:ring-2 focus:ring-[#D8FF6A]/10 ${props.className ?? ''}`}
    />
  )
}
    ...
    <div className="rounded-[30px] border border-white/8 bg-[#171719] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.28)] sm:p-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h3 className="mt-2 text-xl font-black text-white">Before saving</h3>
        </div>
      ...
      <div className="mt-5 overflow-hidden rounded-[24px] border border-white/10 bg-[#FEFDDF] text-[#1c1712]">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {previewSections.length ? (
              previewSections.map((section) => (
                <div key={section.key} className="rounded-[18px] border border-[#73A5CA]/12 bg-white/55 p-3">
              ...
              <div className="rounded-[18px] border border-dashed border-[#73A5CA]/18 bg-white/55 p-4 text-sm text-[#6e6256] sm:col-span-2">
...
export default function AdminPagesPanel() {
  ...
  return (
    <div className="mt-6">
      ...
      {error ? (
        <div className="mt-4 rounded-[20px] border border-[#E87F24]/30 bg-[#2a1a14] px-4 py-3 text-sm text-[#FFBC8C]">
          {error}
        </div>
      ) : null}
      {message ? (
        <div className="mt-4 rounded-[20px] border border-[#D8FF6A]/20 bg-[#D8FF6A]/10 px-4 py-3 text-sm text-[#D8FF6A]">
          {message}
        </div>
      ) : null}
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.02fr_0.98fr]">
        <div className="rounded-[30px] border border-white/8 bg-[#171719] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.28)] sm:p-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-[11px] uppercase tracking-[0.26em] text-white/40">Page editor</div>
              <h3 className="mt-2 text-2xl font-black tracking-[-0.04em] text-white">Selected page</h3>
            </div>
            <button
              type="button"
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <label className="block">
              <div className="mb-2 text-xs uppercase tracking-[0.24em] text-white/56">Slug</div>
              <TextInput value={draft.slug} onChange={(e) => setDraft((prev) => ({ ...prev, slug: e.target.value }))} />
            </label>
            <label className="block">
              <div className="mb-2 text-xs uppercase tracking-[0.24em] text-white/56">Title</div>
              <TextInput value={draft.title} onChange={(e) => setDraft((prev) => ({ ...prev, title: e.target.value }))} />
            </label>
            <label className="block">
              <div className="mb-2 text-xs uppercase tracking-[0.24em] text-white/56">Hero label</div>
              <TextInput value={draft.hero_label} onChange={(e) => setDraft((prev) => ({ ...prev, hero_label: e.target.value }))} />
            </label>
            <label className="block">
              <div className="mb-2 text-xs uppercase tracking-[0.24em] text-white/56">Sort order</div>
              <TextInput
                type="number"
          ...
          <div className="mt-4 grid gap-4">
            <label className="block">
              <div className="mb-2 text-xs uppercase tracking-[0.24em] text-white/56">Hero title</div>
              <TextArea value={draft.hero_title} onChange={(e) => setDraft((prev) => ({ ...prev, hero_title: e.target.value }))} />
            </label>
            <label className="block">
              <div className="mb-2 text-xs uppercase tracking-[0.24em] text-white/56">Hero copy</div>
              <TextArea value={draft.hero_copy} onChange={(e) => setDraft((prev) => ({ ...prev, hero_copy: e.target.value }))} />
            </label>
            <label className="block">
              <div className="mb-2 text-xs uppercase tracking-[0.24em] text-white/56">Hero image URL</div>
            ...
            <div className="rounded-[22px] border border-[#D8FF6A]/12 bg-[#D8FF6A]/8 p-4 text-sm leading-6 text-white/62">
              Page hero fields are edited here. Section content is managed through the
              <span className="font-semibold text-[#D8FF6A]"> Sections Builder </span>
              tab with guided forms for cards, FAQs, sliders, and other page blocks.
            </div>
            <label className="mt-1 flex items-center gap-2 text-sm text-white/65">
              <input
                type="checkbox"
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            <button
              ...
              disabled={saving}
              onClick={savePage}
              className="inline-flex items-center gap-3 rounded-full bg-[#D8FF6A] px-5 py-3 text-sm font-semibold text-[#111111] disabled:cursor-not-allowed disabled:opacity-60"
            >
              Save page
              <ArrowRight size={15} />
            </button>
          ...
          {selectedPage ? (
            <div className="mt-5 rounded-[22px] border border-white/8 bg-white/5 p-4 text-sm text-white/62">
              Editing <span className="font-semibold text-white">{selectedPage.title}</span>
            </div>
          ) : null}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-[30px] border border-white/8 bg-[#171719] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.28)] sm:p-6">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="text-[11px] uppercase tracking-[0.26em] text-white/40">Pages library</div>
            ...
            <div className="mt-5 space-y-3">
                {loading ? (
                  <div className="rounded-[22px] border border-dashed border-white/10 bg-white/5 px-4 py-6 text-sm text-white/50">
                    Loading pages...
                  </div>
                ) : pages.length ? (
                  pages.map((page) => (
                    <button
                      ...
                      className="group flex w-full flex-col gap-4 rounded-[24px] border border-white/8 bg-white/5 p-3 text-left transition hover:-translate-y-0.5 hover:border-[#D8FF6A]/20 hover:bg-white/7 sm:flex-row"
                    >
                    <div className="flex aspect-[16/10] w-full shrink-0 items-center justify-center overflow-hidden rounded-[18px] border border-white/8 bg-[#111111] sm:h-20 sm:w-28 sm:aspect-auto">
                      {page.hero_image ? (
                        <Image src={page.hero_image} alt={page.title} width={112} height={80} className="h-full w-full object-cover" />
                      ) : (
                        <span className="text-[10px] uppercase tracking-[0.24em] text-white/40">No image</span>
                      )}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-3">
                        <h4 className="truncate text-sm font-semibold text-white">{page.title}</h4>
                        <span className="rounded-full border border-white/8 bg-white/5 px-2 py-1 text-[10px] uppercase tracking-[0.22em] text-white/50">
                          {page.slug}
                        </span>
                      </div>
                      <p className="mt-1 line-clamp-2 text-xs leading-5 text-white/55">{page.hero_title}</p>
                      <div className="mt-2 text-[10px] uppercase tracking-[0.24em] text-white/36">
                        {page.hero_label}
                      </div>
                    </div>
                  </button>
          ...
          <div className="rounded-[30px] border border-white/8 bg-[#171719] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.28)] sm:p-6">
            <div className="text-[11px] uppercase tracking-[0.26em] text-white/40">Section data</div>
            <h3 className="mt-2 text-xl font-black tracking-[-0.04em] text-white">Preview-first editing</h3>
