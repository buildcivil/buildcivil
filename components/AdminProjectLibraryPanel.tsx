...
function TextInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className="w-full rounded-[16px] border border-white/10 bg-[#0f0f0f] px-4 py-3 text-sm text-[#F5F3EB] outline-none transition focus:border-[#D8FF6A]/70 focus:ring-2 focus:ring-[#D8FF6A]/10"
    />
  )
}
...
function TextArea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      {...props}
      rows={4}
      className="w-full resize-y rounded-[16px] border border-white/10 bg-[#0f0f0f] px-4 py-3 text-sm leading-6 text-[#F5F3EB] outline-none transition focus:border-[#D8FF6A]/70 focus:ring-2 focus:ring-[#D8FF6A]/10"
    />
  )
}
...
function PreviewImage({ src, alt }: { src: string; alt: string }) {
  if (!src?.trim()) {
    return <div className="flex h-full min-h-[220px] items-center justify-center text-sm text-white/45">Image preview</div>
  }
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
      <div className="rounded-[30px] border border-white/8 bg-[#171719] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.28)] sm:p-6">
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="text-[11px] uppercase tracking-[0.26em] text-white/40">Projects editor</div>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-[22px] border border-white/8 bg-white/[0.03] p-3">
          <div className="grid w-full gap-3 sm:grid-cols-[minmax(220px,1fr)_auto] sm:items-center">
            <label className="block">
              <span className="text-[10px] uppercase tracking-[0.24em] text-white/36">Select project</span>
              <select
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          ...
          <Field label="Category">
            <select value={draft.category} onChange={(event) => setDraft((prev) => ({ ...prev, category: event.target.value as ProjectDraft['category'] }))} className="w-full rounded-[16px] border border-white/10 bg-[#0f0f0f] px-4 py-3 text-sm text-[#F5F3EB] outline-none transition focus:border-[#D8FF6A]/70 focus:ring-2 focus:ring-[#D8FF6A]/10">
      ...
      <div className="space-y-4">
        <div className="rounded-[30px] border border-white/8 bg-[#171719] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.28)] sm:p-6">
          ...
          <h3 className="mt-2 text-xl font-black text-white">{draft.title || 'Project title'}</h3>
          <div className="relative mt-5 h-[360px] overflow-hidden rounded-[28px] border border-white/8 bg-white/5">
            <PreviewImage src={draft.image} alt={draft.title || 'Project preview'} />
          </div>
          <p className="mt-4 text-sm leading-6 text-white/58">{draft.description || 'Project description will appear here.'}</p>
        </div>

        <div className="rounded-[30px] border border-white/8 bg-[#171719] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.28)] sm:p-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-[11px] uppercase tracking-[0.26em] text-white/40">Project library</div>
              <h3 className="mt-2 text-xl font-black tracking-[-0.04em] text-white">Select a project</h3>
            </div>
            <FolderKanban size={18} className="text-[#D8FF6A]" />
          </div>
          <div className="mt-5 space-y-3">
            {projects.length ? projects.map((project) => (
              <button key={project.id} type="button" onClick={() => setDraft(toDraft(project))} className="group flex w-full flex-col gap-4 rounded-[24px] border border-white/8 bg-white/5 p-3 text-left transition hover:-translate-y-0.5 hover:border-[#D8FF6A]/20 hover:bg-white/7 sm:flex-row sm:items-start">
                <div className="relative aspect-[16/10] w-full overflow-hidden rounded-[18px] border border-white/8 sm:h-20 sm:w-28 sm:shrink-0 sm:aspect-auto">
                  <PreviewImage src={project.image} alt={project.title} />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-3">
                    <h4 className="truncate text-sm font-semibold text-white">{project.title}</h4>
                    <span className="rounded-full border border-white/8 bg-white/5 px-2 py-1 text-[10px] uppercase tracking-[0.22em] text-white/50">{project.year}</span>
                  </div>
                  <p className="mt-1 line-clamp-2 text-xs leading-5 text-white/55">{project.description}</p>
                  <div className="mt-2 text-[10px] uppercase tracking-[0.24em] text-white/36">{project.category} • {project.card_label}</div>
                </div>
              </button>
            )) : (
              <div className="rounded-[24px] border border-dashed border-white/10 bg-white/5 p-5 text-sm leading-6 text-white/55">
                <div className="font-semibold text-white">No projects in Supabase yet.</div>
