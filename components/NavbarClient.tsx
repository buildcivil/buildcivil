...
export default function NavbarClient({ settings }: NavbarClientProps) {
  ...
  const navLinks = useMemo(() => settings.navLinks.filter((link) => link.visible), [settings.navLinks])
  const cta = settings.cta
  const mobileTopBar = settings.mobileTopBar
  const showMobileTopBar = Boolean(mobileTopBar?.visible)

  const shellClass = 'border-[#73A5CA]/12 bg-white text-[#1c1712] shadow-[0_12px_28px_rgba(28,23,18,0.08)]'

  const linkClass = 'font-display text-sm uppercase tracking-[0.2em] text-[#1c1712]/82 transition-colors duration-300 hover:text-[#E87F24] xl:text-base'

  const brandPrimary = 'text-[#1c1712]'
  const brandAccent = 'text-[#E87F24]'
  const buttonClass = 'btn-primary hidden min-w-[154px] items-center justify-center gap-3 px-4 py-3 text-sm font-medium tracking-[0.1em] transition-all duration-300 hover:shadow-[0_10px_24px_rgba(232,127,36,0.2)] xl:inline-flex 2xl:min-w-[164px] 2xl:px-5 2xl:tracking-[0.12em]'
  const desktopLogoWidth = scaleDimension(resolveDimension(settings.brand.desktopWidth, 180, 96, 360), 1.5, 540)
  const tabletLogoWidth = scaleDimension(resolveDimension(settings.brand.tabletWidth, 150, 92, 280), 1.5, 420)
  const mobileLogoWidth = scaleDimension(resolveDimension(settings.brand.mobileWidth, 126, 88, 220), 1.5, 330)
  ...
  return (
    <>
        ...
        >
          ...
          >
            {mobileTopBar.locationText ? (
              <span className="min-w-0 truncate text-left leading-tight">{mobileTopBar.locationText}</span>
            ) : <span />}
            {mobileTopBar.phoneLabel ? (
              <a
                href={mobileTopBar.phoneHref || `tel:${mobileTopBar.phoneLabel.replace(/[^\d+]/g, '')}`}
      ...
      >
        <div className={`mx-auto flex w-full max-w-none items-center justify-between border-b px-4 py-2.5 transition-all duration-500 md:px-7 lg:px-10 ${shellClass}`}>
          <a href={settings.brand.href} className="flex items-center gap-3 group">
            {settings.brand.showImageLogo && settings.brand.logoUrl ? (
              <>
                <span className="relative block h-[4.5rem] overflow-hidden xl:hidden" style={{ width: mobileLogoWidth }}>
                  <Image src={settings.brand.logoUrl} alt={settings.brand.logoAlt || 'BuildCivil Constructions'} fill className="object-contain object-left" sizes={`${mobileLogoWidth}px`} priority unoptimized />
                </span>
                <span className="relative hidden h-[5.25rem] overflow-hidden xl:block" style={{ width: desktopLogoWidth }}>
                  <Image src={settings.brand.logoUrl} alt={settings.brand.logoAlt || 'BuildCivil Constructions'} fill className="object-contain object-left" sizes={`${desktopLogoWidth}px`} priority unoptimized />
                </span>
              </>
            ) : (
              <span className="hidden xl:flex items-center gap-3">
                ...
                <span className="flex flex-col leading-[0.78]">
                  <span className={`font-display text-[1.8rem] font-black uppercase tracking-tight lg:text-[2.3rem] ${brandPrimary}`} style={{ color: primaryColor }}>
                    {settings.brand.wordTop}
                  </span>
                  <span className={`font-display text-[1.8rem] font-black uppercase tracking-tight lg:text-[2.3rem] ${brandAccent}`} style={{ color: accentColor }}>
            ...
            {!settings.brand.showImageLogo || !settings.brand.logoUrl ? (
              <span className="flex xl:hidden flex-col font-display text-2xl font-black uppercase tracking-tight leading-none" style={{ color: primaryColor }}>
      ...
      {mobileOpen ? (
        <div className="fixed inset-0 z-[80] flex flex-col overflow-y-auto bg-white text-[#1c1712] transition-all duration-300 xl:hidden">
          <div className="flex min-h-[128px] items-center justify-between gap-5 border-b border-[#1c1712]/10 px-8 py-7 sm:min-h-[150px] sm:px-10">
            <a href={settings.brand.href} onClick={() => setMobileOpen(false)} className="flex min-w-0 items-center gap-4">
              {settings.brand.showImageLogo && settings.brand.logoUrl ? (
                <span className="relative block h-20 shrink-0 overflow-hidden" style={{ width: drawerLogoWidth }}>
                  <Image
                ...
                <>
                  ...
                  >
                    <span className="font-display text-lg font-black uppercase tracking-[-0.08em]" style={{ color: primaryColor }}>
                      {settings.brand.markTop}
                    </span>
                  </span>
                  <span className="flex min-w-0 flex-col font-display text-[1.65rem] font-black uppercase leading-[0.95] tracking-[0.05em]">
                    <span className="truncate" style={{ color: primaryColor }}>{settings.brand.mobileTop || settings.brand.wordTop}</span>
                    <span className="truncate" style={{ color: primaryColor }}>{settings.brand.mobileBottom || settings.brand.wordBottom}</span>
                  </span>
                </>
              )}
            </a>
            ...
            <div className="border-y border-[#1c1712]/10">
              {navLinks.map((link) => (
                ...
                >
                  ...
                  <ChevronRight size={28} className="text-[#1c1712]/45 transition group-hover:translate-x-1 group-hover:text-[#E87F24]" />
                </a>
              ))}
            </div>
